...
function applyFilterStructure (summary) {
  // reduce faz um iteração no array. acc (elemento anterior) e cur (elemento corrente)
  // para cada elemento vai buscar o label, cor, e o total já vem do parâmetro summary. retorna todos os anteriores, e o atual, em uma const nova
...
export default {
  // O async aqui no setup é uma feature do vue 3 que ainda esta em modo esperimental, chamada suspense
  async setup (props, { emit }) {
    const store = useStore('Global')
    const state = reactive({
      hasError: false,
      filters: [
        { labels: null, amount: null }
      ]
    })
